    ...
    def agent_node(state: AgentState):
        ...
        system_prompt = SystemMessage(
            content=(
                ...
                "9. crm_archive_customer / crm_archive_order：当用户要求删除、归档客户或订单时使用。"
                "归档为破坏性操作：必须先向用户展示目标客户/订单信息并取得明确同意，"
                "只有用户明确同意后才以 confirmed=True 调用执行归档；未经确认不得直接归档。"
                "注意：已归档数据在 crm_query_customer / crm_query_order 中会被视为不存在，"
                "请勿先用查询工具确认目标，直接调用归档工具即可。\n"
                "10. crm_restore_customer / crm_restore_order：当用户要求恢复已归档（软删除）的客户或订单时使用。"
                "恢复为写操作：必须先向用户展示目标信息并取得明确同意，"
                "用户同意后才以 confirmed=True 调用执行恢复。"
                "重要：已归档数据在 crm_query_customer / crm_query_order 中会被视为不存在，"
                "因此用户请求恢复时请勿先调用查询/搜索工具（会误报『未找到』），"
                "应直接调用恢复工具。若用户只提供客户姓名或订单产品名（而非 ID），"
                "直接将该姓名/产品名作为 keyword 参数传入恢复工具即可，工具会自动在含已归档数据中匹配。\n"
                "11. crm_delete_customer / crm_delete_order：当用户要求彻底删除（物理删除）已归档的客户或订单时使用。"
                "彻底删除为极其危险的破坏性操作且不可恢复，仅允许对已归档的数据操作："
                "必须先向用户展示目标信息并明确提示不可恢复，取得用户明确同意后，"
                "才以 confirmed=True 调用执行彻底删除；未经确认不得执行。"
                "同样地，已归档数据在查询工具中不可见，请勿先查询，直接调用删除工具定位目标。\n\n"
                + kb_context
